...
function coin_enters_target(ev){
    ev.target.classList.add("shining");
...
function coin_drop(ev){
    ...
    document.getElementById("debug_coin_state_display").innerHTML = ev.target.className;
    if(ev.target.className == "drop_target"){
...
function coin_leaves_target(ev){
    ev.target.classList.remove("shining");
}
